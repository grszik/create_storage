...
public class Fluids {
    ...
    public static final FluidContainer CAKE_BATTER = new FluidContainer("cake_batter", FluidType.Properties.create().canSwim(false).canDrown(true).canPushEntity(true).supportsBoating(false).viscosity(2), () -> FluidContainer.createExtension(new FluidContainer.ClientExtensions(CreateSNTE.MODID, "cake_batter").overlay(null).tint(0xFF7C2B).still("cake_batter").flowing("cake_batter_flow").overlay("cake_batter")), BlockBehaviour.Properties.copy(Blocks.WATER));
}
